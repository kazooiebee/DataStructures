...
void print_path_grid(vector<vector<bool> > const& blocked_grid, std::vector<Point> points) {
...
bool num_paths(int x, int y, vector<vector<bool> > & blocked_grid, vector<Point> points){
     Point a = Point(x,y);
     points.push_back(a);
     ...
     if(x == 0 && y == 0){
          ...
          print_path_grid(blocked_grid, points);
     }

     if(x==0){
          if(!blocked_grid[x][y-1]){
               return num_paths(x, y-1, blocked_grid, points);
          }
          else{
               return 0;
          }
     ...
     if(y==0){
          if(!blocked_grid[x-1][y]){
               return num_paths(x-1, y, blocked_grid, points);
          }
          else{
               return 0;
          }
     ...
     if(!blocked_grid[x-1][y] && !blocked_grid[x][y-1]){
          bool success = num_paths(x-1, y, blocked_grid, points);
          if(!success){
               return num_paths(x, y-1, blocked_grid, points);
          }
          return success;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Number of paths: " << num_paths(start_x, start_y, blocked_grid, points) << std::endl;
