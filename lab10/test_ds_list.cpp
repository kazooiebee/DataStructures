...
int main() {
  ...
  std::cout << "sanity check: " << set3.sanity_check() << std::endl;
  set3.print_as_sideways_tree( std::cout );


     tree_iterator<int> itr = set3.begin();
     for (int i = 0; i < set3.size(); i++){
          std::cout << "before dec: " <<*itr;
          itr++;
          itr--;
          std::cout << "      after dec: " << *itr <<std::endl;
          itr++;
     }
   ...
   tree_iterator<int> itr2 = set4.begin();
   for (int i = 0; i < set4.size(); i++){
        std::cout << "before dec: " <<*itr2;
        itr2++;
        itr2--;
        std::cout << "      after dec: " << *itr2 <<std::endl;
        itr2++;
   }

  std::cout << "Accumulate test: " << set1.accumulate("") << std::endl;
  std::cout << "Accumulate test: " << set3.accumulate(0) << std::endl;
  std::cout << "Accumulate test: " << set1.accumulate("abc") << std::endl;
  std::cout << "Accumulate test: " << set3.accumulate(5) << std::endl; 
