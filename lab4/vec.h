...
template <class T> void Vec<T>::resize_smaller(){
     while(m_size!=0&&m_size<=m_alloc/2){
...
template <class T> int remove_matching_elements(Vec<T>& v, const T& value) {
     int sum = 0;
...
template <class T> void Vec<T>::print() const{
     std::cout << "m_data is " << m_data << std::endl;
